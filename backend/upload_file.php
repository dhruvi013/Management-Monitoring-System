...
/*
------------------------------------------------------
 XLSX → CSV-Array Converter (NO external libraries)
------------------------------------------------------
*/
...
$required = ['gr no', 'roll no', 'semester', 'class', 'student name'];
...
try {
    ...
    for ($i = 1; $i < count($data); $i++) {
        ...
        $semester      = trim($row[$map['semester']] ?? '');
        $class         = trim($row[$map['class']] ?? '');
        $full_name     = trim($row[$map['student name']] ?? '');

        if ($gr_no === '' || $enrollment_no === '' || $class === '' || $semester === '' || $full_name === '') {
            continue;
        }
